...
func (c *DPFMAPICaller) deleteSqlProcess(
	...
	var incotermsData *dpfm_api_output_formatter.Incoterms
	for _, a := range accepter {
		switch a {
		case "Incoterms":
			h := c.incotermsDelete(input, output, log)
			incotermsData = h
			if h == nil {
				continue
			}
	...
	return &dpfm_api_output_formatter.Message{
		Incoterms: incotermsData,
	}
}

func (c *DPFMAPICaller) incotermsDelete(
	input *dpfm_api_input_reader.SDC,
	output *dpfm_api_output_formatter.SDC,
	log *logger.Logger,
) *dpfm_api_output_formatter.Incoterms {
	sessionID := input.RuntimeSessionID
	incoterms := c.IncotermsRead(input, log)
	incoterms.IsMarkedForDeletion = input.Incoterms.IsMarkedForDeletion
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": incoterms, "function": "IncotermsIncoterms", "runtime_session_id": sessionID})
	if err != nil {
	...
	if !checkResult(res) {
		...
		output.SQLUpdateError = "Incoterms Data cannot delete"
		return nil
	}
